...
parser.add_argument('--chk_path', default="checkpoint/cifar10/new_hypers/cnns", type=str, help='path to save the checkpoints')
parser.add_argument('--result_folder', default='./results/cifar10/new_hypers/cnns/', type=str, help='path to save the results')
...
def train_model(depth,
                ...
                hyperupdates = 100,
                batchnorm = False,
                chk_path = "checkpoint/cifar10/optimized_la",
                result_folder = './results/cifar10/optimized_la/',
                debug=False
               ):
    """
    ...
    if optimizehypers:
        path = "cifar10_" + str(prior_prec_init) + '_fixup_cnn' + str(depth) + '_' + str(width) + '_' + prior_structure + '_' + hessian_structure + '_hlr' + str(hypers_lr) + '_baselr_' + str(base_lr)
    else:
        path = "cifar10_" + str(prior_prec_init) + '_fixup_cnn' + str(depth) + '_' + str(width) + '_' + prior_structure + '_baselr_' + str(base_lr)
    ...
    net = new_models.FixupCNN(in_pixels=32, in_channels=3, n_out=10, width=width, depth=depth)
...
widths_cifar10_cnn = [4, 8, 16, 32, 64]
depths_cifar10_cnn = [1, 2, 3, 4, 5]


for depth in depths_cifar10_cnn:
    for width in widths_cifar10_cnn:
        train_model(depth,
                    width,
                    args.decay,
