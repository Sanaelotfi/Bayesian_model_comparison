...
parser.add_argument('--use_sgdr', default=True, type=bool, help='whether or not to use cosine lr scheduler')
...
parser.add_argument('--chk_path', default="checkpoint/cifar10/new_hypers", type=str, help='path to save the checkpoints')
parser.add_argument('--result_folder', default='./results/cifar10/new_hypers/', type=str, help='path to save the results')
...
def train_model(arch,
    ...
    if optimizehypers:
        path = "cifar10_" + str(prior_prec_init) + '_' + arch + '_' + str(width) + '_' + prior_structure + '_' + hessian_structure + '_hlr' + str(hypers_lr)
    else:
